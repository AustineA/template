class AdminsController < ApplicationController
  ...
  def search_user
    if current_user.admin
      ...
      @users = User.search query, order: {created_at: :desc}, fields: [:company, :f_name, :l_name, :email, :phone, :username], misspellings: {edit_distance: 3, below: 2},  where: args, aggs: { account_type: {}, posts_count: {} }, page: params[:page], per_page: 12
      render :users
    else
      render json:   { message: "You're not authorized to access this resource" }, status: :unauthorized
    end
